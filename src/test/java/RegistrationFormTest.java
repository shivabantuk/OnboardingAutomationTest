...
public class RegistrationFormTest {
    ...
    public void testInvalidUsername() {
        ...
        lastName.sendKeys("Doe");
        username.sendKeys("jane@doe"); // Invalid username with special characters
        password.sendKeys("password123");
        submitButton.click();

        WebElement usernameError = driver.findElement(By.id("username_error"));
        Assert.assertTrue(usernameError.isDisplayed(), "Username error should be displayed");
        Assert.assertEquals(usernameError.getText(), "Invalid username format.", "Incorrect username error message");
    }
}